...
const addAliaseTypesToTree = ({ schema }) => {
    Object.entries(schema.aliases).forEach(([alias, path]) => {
        lodash_1.default.set(schema.tree, `${alias}._aliasRootField`, lodash_1.default.get(schema.tree, path));
    });
};
const parseChildSchemas = ({ schema, isDocument, noMongoose, modelName, datesAsStrings }) => {
    var _a, _b, _c, _d, _e;
    let childInterfaces = "";
    ...
    const processChild = (rootPath) => {
        return (child) => {
            ...
            const isSchemaMap = (_a = child.model.$isSchemaMap) !== null && _a !== void 0 ? _a : false;
            ...
            if (lodash_1.default.get(schema.tree, requiredValuePath) === true) {
            ...
            if (isSubdocArray) {
                ...
                if (lodash_1.default.has(schema.tree, defaultValuePath) &&
                    lodash_1.default.get(schema.tree, defaultValuePath) === undefined) {
                    child.schema._isDefaultSetToUndefined = true;
                }
            }
            if (isSchemaMap) {
                lodash_1.default.set(schema.tree, path, { type: Map, of: isSubdocArray ? [child.schema] : child.schema });
            }
            else if (isSubdocArray) {
                lodash_1.default.set(schema.tree, path, [child.schema]);
            }
            else {
                lodash_1.default.set(schema.tree, path, child.schema);
            }
            ...
            if (isDocument) {
                ...
                // not sure why schema doesnt have `tree` property for typings
                    ...
                    if (schema.tree._id)
                        _idType = (0, exports.convertBaseTypeToTs)({
                            key: "_id",
                            val: schema.tree._id,
                            isDocument: true,
                            noMongoose,
...
const getParseKeyFn = (isDocument, shouldLeanIncludeVirtuals, noMongoose, datesAsStrings) => {
    return (key, valOriginal) => {
        ...
        if (BASE_TYPES.has(val))
            val = { type: val };
        const isMap = isMapType(val === null || val === void 0 ? void 0 : val.type);
        ...
        if (val === Array || (val === null || val === void 0 ? void 0 : val.type) === Array || isUntypedArray) {
            ...
            if (val._aliasRootField) {
                const parseKey = (0, exports.getParseKeyFn)(isDocument, shouldLeanIncludeVirtuals, noMongoose, datesAsStrings);
...
const parseSchema = ({ schema: schemaOriginal, modelName, isDocument, header = "", footer = "", noMongoose, datesAsStrings, shouldLeanIncludeVirtuals }) => {
    let template = "";
    ...
    if (!lodash_1.default.isEmpty(schema.aliases) && modelName) {
